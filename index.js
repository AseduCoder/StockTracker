...
function fetchTopStocks() {
  fetch(
    "https://www.alphavantage.co/query?function=SECTOR&apikey=RNZJ02TA282RUAOB"
  )
...
function fetchStockData(symbol) {
  ...
  fetch(
    `https://www.alphavantage.co/query?function=GLOBAL_QUOTE&symbol=${symbol}&apikey=RNZJ02TA282RUAOB`
  )
    .then((response) => response.json())
    .then((data) => {
      ...
      if (quote && quote["10. change percent"]) {
        ...
        stockList.innerHTML = '<li class="error">Sory , Invalid Symbol</li>';
      }
    })
    .catch((error) => console.error(error));
}
